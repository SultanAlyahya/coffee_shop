...
def post_drinks(jwt):
    print(isinstance(request.get_json()['recipe'], list))
    recipe = request.get_json()['recipe']
    if not isinstance(request.get_json()['recipe'], list):
        recipe = [recipe]
    recipeTOString = str(recipe).replace("\'", "\"")
    Drink(title=request.get_json()['title'], recipe=recipeTOString).insert()
    print(str(request.get_json()['recipe']).replace("\'", "\""))
    drinks = Drink.query.all()
    print(drinks)
    formatted_drinks = [drink.long() for drink in drinks]
    return jsonify({"success": True, "drinks": formatted_drinks})
